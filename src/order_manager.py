...
# Imports
from src.whatsapp_client import send_message, download_voice_note, mark_as_read
from src.transcriber import transcribe
from src.router import classify_intent
from src.extractor import extract_order
from src.sheets import append_order, answer_question
...
def process_message(sender: str, msg_type: str, msg_data: dict):
    ...
    try:
        ...
        if is_message_processed(msg_data["id"], sender):
            print(f"[DEDUPLICATION] Message {msg_data['id']} from {sender} was already processed. Skipping duplicate.", flush=True)
            return
        ...
        if intent == "ORDER":
            ...
            try:
                ...
                if str(e) == "DUPLICATE_ORDER":
                    send_message(sender, f"📋 Yeh order pehle se save hai! (Order ID: {order.order_id}) 😊")
                    return
                raise e
            ...
            if m.chest: reply += f"  • Chhati: {m.chest}\"\n"
            if m.waist: reply += f"  • Kamar: {m.waist}\"\n"
            if m.hip: reply += f"  • Hip: {m.hip}\"\n"
            if m.shoulder: reply += f"  • Kandha: {m.shoulder}\"\n"
            if m.sleeve_length: reply += f"  • Baju: {m.sleeve_length}\"\n"
            if m.garment_length: reply += f"  • Lambai: {m.garment_length}\"\n"
            if m.salwar_length: reply += f"  • Salwar Len: {m.salwar_length}\"\n"
            if m.mori: reply += f"  • Mori: {m.mori}\"\n"
        ...
        send_message(sender, "😅 Maaf kijiye, kuch gadbad ho gayi. Ek baar phir se bolenge?")
